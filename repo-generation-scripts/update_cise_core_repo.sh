...
mvn -U org.apache.maven.plugins:maven-dependency-plugin:2.9:copy-dependencies -DincludeGroupIds=eu.europa.ec.jrc.marex,eu.cise.adaptor -DoutputDirectory=$DUMMY_REPO -Dmdep.copyPom=true -Dmdep.addParentPoms=true -DoverWriteReleases=true
...
for pomFile in $(find $DUMMY_REPO -type f -name \*.pom); do
   ...
   mvn deploy:deploy-file -DgroupId=$groupId -DartifactId=$artifactId -Dversion=$version -Durl=file:$TARGET_REPO/releases -DrepositoryId=file-releases -DupdateReleaseInfo=true -Dfile=$mavenJarFile -DpomFile=$pomFile -DcreateChecksum=true -Dclassifier=javadoc
done
